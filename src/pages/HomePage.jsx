              ...
              <div className="flex flex-1 flex-col">
                ...
                <CardContent className="flex flex-col gap-4 p-5 pb-4 pt-1">
                  {todayTasks.length === 0 ? (
                    ...
                    <div className="max-h-[80px] space-y-2 overflow-y-auto custom-scrollbar">
                      {todayTasks.slice(0, 3).map(task => (
                        <div key={task.id} className="flex items-center gap-2 text-sm text-card-foreground">
                          <span className={`h-1.5 w-1.5 rounded-full ${task.done ? 'bg-muted-foreground' : 'bg-primary'}`} />
                          <span className={task.done ? 'line-through text-muted-foreground' : ''}>{task.title}</span>
                        </div>
                      ))}
                      {todayTasks.length > 3 && (
                        <div className="pt-1 text-[10px] text-muted-foreground">+{todayTasks.length - 3} more</div>
                      )}
                    </div>
                  )}
                </CardContent>
